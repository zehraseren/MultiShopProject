...
namespace MS.Image.WebUI.Services;
...
public class CloudStorageService : ICloudStorageService
{
    ...
    public async Task<string> GetSignedUrlAsync(string fileNameToRead, int timeOutInMinutes = 30)
    {
        try
        {
            ...
            // prodives limited permission and time to make a request: time here is mentioned for 30 minutes
        ...
        {
            _logger.LogError($"Error occured while getting signed url for file {fileNameToRead}: {ex.Message}");
            throw;
        }
    }
